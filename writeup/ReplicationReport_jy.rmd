...
The figures below show results from the reproduction. The significance thresholds are uncorrected for multiple comparisons in order to visualize the regions of grey matter volume change. Two brain figures and results tables are shown for both VBM8 and CAT12 pipelines. They represent the contrasts from the t-test: controls < users and controls > users. The red arrow indicates where the original cerebellar finding was located approximately. Although there were no significant clusters after FWE-correction, it is interesting that the two pipelines produced different clusters from the same data. 

### Confirmatory analysis

I could not reproduce the key statistic: pFWE = .035. The results with the lowest p-values in the CB group (users) greater than HC group (ctrls) contrast were:
...
For further analysis, I administered the pipelines on the follow-up data. Neither the VBM8 nor CAT12 versions yielded significant results after FWE-corrections. See figures below:
...
The mean correlation analysis suggests both versions of the SPM VBM pipeline can produce high quality tissue segmentation and template registration maps. The VBM8 method resulted in two volumes with mean correlations over two standard deviations away from the group average; the CAT12 method had one. 
...
*I think one of the main reasons why this pipeline reproduction failed was due to a lack of detailed information on pipeline settings.*

One clear takeway from this replication attempt is that neuroimaging methods are more complicated than what is typically described in a manuscript. The methods quoted from the original paper are not sufficient to reproduce the analysis. I initially reviewed their original paper, and then I went to review other VBM papers that used SPM. I ultimately got the most help from consulting the SPM8 and SPM12 instruction manuals for VBM8 and CAT12, watching SPM vbm tutorials on Youtube, and using the SPM user forum. 

The majority of the pipeline is image (pre)-processing. This information, although vital to the analysis, is not very interesting to readers, so it makes sense that it gets left out in the final manuscripts. Unfortunately, this absence combined with the neuroimaging's lack of processing standards, makes reproducibility a challenge in the field. Luckily, things like fMRIprep and the BIDS data standard are helping everyone get onto the processing standard. I think that this will help progress the field significantly.
